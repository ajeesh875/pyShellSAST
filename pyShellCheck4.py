...
def analyze_shell_script(script_path, regex_config, output_file, max_processes=None):
    try:
        ...
        with multiprocessing.Pool(processes=max_processes) as pool:
            # Create a list of argument sets for starmap
            argument_sets = []
            for pattern, description in regex_config:
                print(pattern)
                compiled_pattern = re.compile(pattern)
                argument_sets.append((script_content, compiled_pattern, description))

            # Use starmap to execute analyze_vulnerability in parallel
            results = pool.starmap(analyze_vulnerability, argument_sets)
